...
function mixed_subdivision(A::Matrix{Int}, V::Matrix{C}) where C
    ...
    ms_test = gather_mixed_cells(wd.M, wd)
    test_lift = test_vector(p1)
    display(A_ext)
    println(tvol)
    println(sum([vol(m, A_ext) for m in ms_test]))
    for m in ms_test
        println(m.inds)
        println(vol(m, A_ext))
        @assert is_dual_tropical_root(wd, m, test_lift)
    end

    A_size = size(A, 2)
    M_final = MCI(wd.M.V[:, 1:A_size], wd.M.A_modP[:, 1:A_size], wd.M.A_Fl[:, 1:A_size])
    return Lift(p1.r[1:A_size], p1.eps[1:A_size]), gather_mixed_cells(M_final, wd, A_size)
end
